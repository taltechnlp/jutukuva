<script lang="ts">
    ...
    import {EndSessionModal, ShareSessionModal, SpeechEditor} from '$lib/components/prosemirror-speech';
    import AudioSourceManager, {type AudioDevice, type AudioSourceType} from '$lib/audioSourceManager';
    ...
    import type {Participant, SessionInfo} from '$lib/collaboration/types';
    ...
    let speechEndTimer: NodeJS.Timeout | undefined;
    ...
    let postSpeechTimer: NodeJS.Timeout | undefined;
    ...
    async function transcribeFile(file: File) {
        ...
        try {
            ...
            // 2. Clear any previous transcript so the new file starts fresh.
            speechEditor?.clear();
            speechEditor?.startTiming();
        ...
        <div class="flex w-full justify-center items-center gap-3">
            ...
            {#if connectionError}
                <div class="badge badge-error gap-2">
                    ...
                    <span class="truncate max-w-50">{connectionError}</span>
            ...
            {:else if isRecording}
                <div class="badge badge-error gap-2 animate-pulse">
                    ...
                    <span class="truncate max-w-50">{isSpeaking ? $_('dictate.speakingDetected') : $_('dictate.listeningForSpeech')}</span>
                </div>
            {:else if isTranscribingFile}
                <div class="badge badge-info gap-2">
                    <span class="loading loading-spinner loading-xs shrink-0"></span>
                    <span class="truncate max-w-60">{$_('dictate.transcribingFile', {default: 'Transcribing file...'})} {fileTranscriptionProgress}
                        %</span>
                </div>
            {:else if isWasmLoading || !isWasmReady}
                <div class="badge badge-info gap-2">
                    <span class="loading loading-spinner loading-xs shrink-0"></span>
                    <span class="truncate max-w-50">{initializationStatus}</span>
                </div>
            ...
            {#if sessionInfo}
                <div class="badge badge-secondary gap-2">
                    ...
                    <button
                            ...
                            onclick={() => {
							if (sessionInfo) navigator.clipboard.writeText(sessionInfo.code);
						}}
                ...
                {#key sessionInfo?.code || 'solo'}
                    ...
                    >
                        {#snippet toolbarCenterContent()}
                            ...
                            {:else}
                                <button class="btn btn-circle btn-error btn-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200"
                                        onclick={stopRecording} title={$_('dictate.stopRecording')}
                            ...
                            <!-- Transcribe Audio/Video File Button -->
                            <input
                                    type="file"
                                    accept="audio/*,video/*,.mp3,.wav,.m4a,.ogg,.flac,.aac,.opus,.mp4,.mov,.webm,.mkv,.m4v"
                                    class="hidden"
                            ...
                            {#if isTranscribingFile}
                                ...
                                <span class="text-xs font-mono opacity-70 min-w-[3ch]">{fileTranscriptionProgress}
                                    %</span>
                            {:else}
                                <button
                                        ...
                                        title={$_('dictate.transcribeFile', { default: 'Transcribe audio or video file' })}
                                        aria-label={$_('dictate.transcribeFile', { default: 'Transcribe audio or video file' })}
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                         viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                              d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 7.5m0 0L7.5 12m4.5-4.5v12.75"/>
                                    </svg>
                                </button>
                            {/if}
                        ...
                        {#snippet toolbarRightContent()}
                            ...
                            <div class="relative">
                                <button type="button"
                                        ...
                                        title={$_('collaboration.collaborative_session')}
                                        onclick={() => { showCollabMenu = !showCollabMenu; if (showCollabMenu) loadCollaborationSessions(); }}>
                                ...
                                {#if showCollabMenu}
                                    <div class="absolute right-0 top-full z-100 p-3 shadow-xl bg-base-100 rounded-box min-w-80 max-w-96 border border-base-300 mt-2 max-h-[70vh] flex flex-col overflow-x-hidden"
                                         use:clickOutside={() => showCollabMenu = false}>
                                        {#if !sessionInfo}
                                            <ul class="menu p-0">
                                                <li>
                                                    <h3 class="menu-title text-sm font-semibold">{$_('collaboration.collaborative_session')}</h3>
                                                </li>
                                                <li>
                                                    <button class="gap-3 cursor-pointer"
                                                            onclick={() => { showCollabMenu = false; startCollaborativeSession(); }}>
                                                    ...
                                                    <ul class="menu p-0">
                                                        ...
                                                        {:else}
                                                            {#each plannedAndActiveSessions as session (session.id)}
                                                                <li>
                                                                    <button class="flex flex-col items-start gap-0.5 py-2 cursor-pointer"
                                                                            onclick={() => { showCollabMenu = false; startCollaborativeSession(session.id); }}>
                                                                        ...
                                                                        <div class="flex items-center gap-3 text-xs opacity-70 w-full">
                                                                            ...
                                                                            {#if session.scheduled_date}
                                                                                <span>{new Date(session.scheduled_date).toLocaleDateString(undefined, {
                                                                                    month: 'short',
                                                                                    day: 'numeric',
                                                                                    hour: '2-digit',
                                                                                    minute: '2-digit'
                                            ...
                                            <ul class="menu p-0">
                                                <li>
                                                    <button class="gap-3 cursor-pointer"
                                                            onclick={() => { showCollabMenu = false; modalStore.openSessions(); }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                  stroke-width="2"
                                        ...
                                        {:else}
                                            <ul class="menu p-0">
                                                <li>
                                                    <h3 class="menu-title text-sm font-semibold">{$_('collaboration.session_active')}</h3>
                                                </li>
                                                <li>
                                                ...
                                                <li>
                                                    <button class="gap-3 cursor-pointer"
                                                            onclick={() => { showCollabMenu = false; showShareModal = true; }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                  stroke-width="2"
                                                ...
                                                <li>
                                                    <button class="gap-3 cursor-pointer"
                                                            onclick={() => { showCollabMenu = false; disconnectCollaboration(); }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                  stroke-width="2"
                                                ...
                                                <li>
                                                    <button class="text-error gap-3 cursor-pointer"
                                                            onclick={() => { showCollabMenu = false; showEndSessionModal = true; }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                                                             fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="relative">
                                <button type="button"
                                        class="btn btn-ghost btn-circle hover:bg-base-200 transition-colors"
                                        title={$_('dictate.audioSource')} onclick={() => showSettings = !showSettings}>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                         viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                {#if showSettings}
                                    <ul class="absolute right-0 top-full z-100 menu p-3 shadow-xl bg-base-100 rounded-box w-72 border border-base-300 mt-2"
                                        use:clickOutside={() => showSettings = false}>
                                        <li>
                                            <h3 class="menu-title text-sm font-semibold">{$_('dictate.audioSource')}</h3>
                                        </li>
                                        ...
                                        <li class="mb-2">
                                            ...
                                            >
                                                <option value="microphone">
                                                    🎤 {$_('dictate.audioSourceMicrophone')}</option>
