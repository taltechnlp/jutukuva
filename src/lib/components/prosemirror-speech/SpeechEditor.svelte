<script lang="ts">
    ...
    import {keyboardShortcutsPlugin, speakerDropdownPlugin} from './plugins/keyboardShortcuts';
    import {insertStreamingTextCommand, signalVadSpeechEndCommand, streamingTextPlugin} from './plugins/streamingText';
    import {type TextSnippetEntry, textSnippetsPlugin, updateTextSnippetEntries} from './plugins/textSnippets';
    ...
    // Public API: Replace the doc with a fresh empty one and reset ASR state.
    export function clear() {
        if (!editorView) return;
        const emptyDoc = speechSchema.node('doc', null, [speechSchema.node('paragraph', null, [])]);
        const tr = editorView.state.tr.replaceWith(0, editorView.state.doc.content.size, emptyDoc.content);
        editorView.dispatch(tr);
        resetAsrContext();
    }
